...
resource "google_compute_instance" "sonarqube" {
  ...
  metadata_startup_script = "sudo apt-get update -y;sudo apt-get install git -y; sudo git clone https://github.com/Diksha86/apachesolr.git; cd /apachesolr; sudo chmod 777 /apachesolr/*; sudo sh solr.sh"
}
